...
static void wait_input_clear(void) {
    for (int i = 0; i < 100000; i++) {
        if (!(inb(PS2_STATUS_PORT) & PS2_STATUS_IN_FULL)) return;
    }
}
...
static mouse_event_t decode_packet(const uint8_t p[3]) {
    ...
    int16_t dx = p[1];
    int16_t dy = p[2];
    if (p[0] & PACKET_X_SIGN) dx -= 256;
    if (p[0] & PACKET_Y_SIGN) dy -= 256;

    if (p[0] & PACKET_X_OVERFLOW) dx = 0;
    if (p[0] & PACKET_Y_OVERFLOW) dy = 0;
...
void ps2_mouse_init(void) {
    ...
    controller_read_data(); // discard ACK
...
bool ps2_mouse_selftest(void) {
    ...
    if (!ev.left_button) return false;
    if (ev.dx != -10) return false;
    if (ev.dy != 20) return false;
    if (ev.right_button || ev.middle_button) return false;
