...
static conversation_state_t status[LOCKBOX6_CAPACITY];

static uint32_t expected_seq[LOCKBOX6_CAPACITY];

static uint32_t our_isn_table[LOCKBOX6_CAPACITY];

static uint32_t our_fin_seq[LOCKBOX6_CAPACITY];
static uint32_t time_wait_started[LOCKBOX6_CAPACITY];

static uint16_t peer_window[LOCKBOX6_CAPACITY];
static uint32_t send_seq[LOCKBOX6_CAPACITY];
...
static void enter_time_wait(uint32_t conn_id)
{
    ...
    kprintf("[Rapport6] slot %d: both sides done, waiting out TIME_WAIT before the slot is free\n", conn_id);
}
...
void rapport6_on_syn(uint32_t conn_id, uint32_t peer_isn, uint32_t our_isn)
{
    ...
    kprintf("[Rapport6] slot %d: said hi, waiting to hear back (peer_isn=%u, our_isn=%u)\n", conn_id, peer_isn, our_isn);
}
...
void rapport6_initiate_close(uint32_t conn_id, uint32_t fin_seq)
{
    ...
    if (status[conn_id] == CONV_ESTABLISHED)
    {
        ...
        kprintf("[Rapport6] slot %d: closing our end, said our goodbyes, waiting for a reply\n", conn_id);
    }
    else if (status[conn_id] == CONV_CLOSE_WAIT)
    {
        status[conn_id] = CONV_LAST_ACK;
        kprintf("[Rapport6] slot %d: peer already said goodbye, now we're saying ours\n", conn_id);
    }
}
...
void rapport6_on_ack(uint32_t conn_id, uint32_t ack_num)
{
    ...
    case CONV_SYN_RECEIVED:
        ...
        kprintf("[Rapport6] slot %d: we're officially talking now\n", conn_id);
        break;
    ...
    case CONV_FIN_WAIT_1:
        if (ack_num >= our_fin_seq[conn_id] + 1)
        {
            ...
            kprintf("[Rapport6] slot %d: our goodbye was heard, waiting for theirs\n", conn_id);
        }
        break;
    ...
    case CONV_LAST_ACK:
        if (ack_num >= our_fin_seq[conn_id] + 1)
        {
            kprintf("[Rapport6] slot %d: our goodbye was heard, letting go of the slot\n", conn_id);
            status[conn_id] = CONV_CLOSED;
            scheduler6_cancel(conn_id);
            lockbox6_release(conn_id);
        }
        break;
...
void rapport6_on_fin(uint32_t conn_id)
{
    ...
    case CONV_ESTABLISHED:
        ...
        kprintf("[Rapport6] slot %d: peer said their goodbyes, we're still allowed to talk until we say ours\n", conn_id);
        break;

    case CONV_FIN_WAIT_1:
        expected_seq[conn_id]++;
        status[conn_id] = CONV_CLOSING;
        kprintf("[Rapport6] slot %d: both sides said goodbye at once\n", conn_id);
        break;
    ...
    case CONV_CLOSED:
        kprintf("[Rapport6] slot %d: already said goodbye once, ignoring the echo\n", conn_id);
        break;

    default:
        kprintf("[Rapport6] slot %d: goodbye received again mid-close, ignoring the echo\n", conn_id);
        break;
    }
}
...
void rapport6_on_rst(uint32_t conn_id)
{
    ...
    {
        kprintf("[Rapport6] slot %d: already hung up on once, ignoring the echo\n", conn_id);
        return;
    }

    kprintf("[Rapport6] slot %d: got hung up on, letting go of the slot\n", conn_id);
    status[conn_id] = CONV_CLOSED;
    scheduler6_cancel(conn_id);
    lockbox6_release(conn_id);
}
...
void rapport6_tick(void)
{
    ...
    for (uint32_t i = 0; i < LOCKBOX6_CAPACITY; i++)
    {
        ...
        if (now - time_wait_started[i] < CONV6_TIME_WAIT_TICKS)
            continue;

        kprintf("[Rapport6] slot %u: TIME_WAIT expired, letting go of the slot\n", i);
...
void rapport6_initiate_connect(uint32_t conn_id, uint32_t our_isn)
{
    ...
    kprintf("[Rapport6] slot %d: reaching out, said hi, waiting for a reply (our_isn=%u)\n", conn_id, our_isn);
}
...
int rapport6_on_syn_ack(uint32_t conn_id, uint32_t peer_isn, uint32_t ack_num)
{
    ...
    {
        kprintf("[Rapport6] slot %d: syn-ack didn't acknowledge our syn, ignoring\n", conn_id);
        return 0;
    }
    ...
    kprintf("[Rapport6] slot %d: they said hi back, we're officially talking now\n", conn_id);
...
int rapport6_send_allowed(uint32_t conn_id, uint16_t length)
{
    ...
    uint32_t in_flight = scheduler6_bytes_in_flight(conn_id);
