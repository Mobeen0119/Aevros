...
static void get_identity6(uint8_t our_mac[6], uint8_t our_ip[16])
{
    memcpy(our_mac, frontdesk_get_state()->mac, 6);
...
int frontdoor_socket(int type)
{
    ...
    if (type != SOCK_TYPE_TCP && type != SOCK_TYPE_UDP && type != SOCK_TYPE_TCP6)
        return -1;
    ...
    else if (type == SOCK_TYPE_TCP6)
        sockets[slot].kind = SOCK_TCP6;
    else
        sockets[slot].kind = SOCK_UDP;

    sockets[slot].listening = 0;

    sockets[slot].conn_id = (sockets[slot].kind == SOCK_TCP6) ? LOCKBOX6_CAPACITY : LOCKBOX_CAPACITY;
...
int frontdoor_bind(int fd, uint16_t port)
{
    ...
    if (sockets[fd].kind == SOCK_TCP6)
    {
        if (!switchboard6_bind(port))
            return -1;
...
int frontdoor_connect6(int fd, const sock_addr6_t *addr)
{
    if (fd < 0 || fd >= FRONTDOOR_MAX_SOCKETS || !sockets[fd].in_use || sockets[fd].kind != SOCK_TCP6)
        return -1;

    uint8_t our_mac[6], our_ip[16];

    get_identity6(our_mac, our_ip);

    uint32_t conn_id;

    if (!switchboard6_connect(addr->ip, addr->port, our_mac, our_ip, &conn_id))
        return -1;
...
int frontdoor_accept(int fd)
{
    ...
    if (sockets[fd].kind == SOCK_TCP6)
    {
        uint32_t conn_id = switchboard6_accept(sockets[fd].local_port);
        if (conn_id == SWITCHBOARD6_NONE)
            return -1;
...
int frontdoor_send(int fd, const uint8_t *buf, uint16_t len)
{
    ...
    if (sockets[fd].kind == SOCK_TCP6)
    {
        ...
        if (!switchboard6_send(sockets[fd].conn_id, buf, len, our_mac, our_ip))
            return -1;
...
int frontdoor_recv(int fd, uint8_t *buf, uint16_t max_len)
{
    ...
    if (sockets[fd].kind == SOCK_TCP6)
    {
        uint16_t got = switchboard6_recv(sockets[fd].conn_id, buf, max_len);

        if (got > 0)
            return got;

        conversation_state_t st = rapport6_get_state(sockets[fd].conn_id);
...
int frontdoor_sendto6(int fd, const sendto6_args_t *args)
{
    if (fd < 0 || fd >= FRONTDOOR_MAX_SOCKETS || !sockets[fd].in_use || sockets[fd].kind != SOCK_UDP)
        return -1;
    ...
    if (!switchboard6_send_udp(sockets[fd].local_port, args->dest.ip, args->dest.port, our_mac, our_ip, args->buf, args->len))
        return -1;
...
int frontdoor_close(int fd)
{
    ...
    if (sockets[fd].kind == SOCK_TCP6 && !sockets[fd].listening && sockets[fd].conn_id != LOCKBOX6_CAPACITY)
    {
        conversation_state_t st = rapport6_get_state(sockets[fd].conn_id);
        ...
        if (st == CONV_ESTABLISHED || st == CONV_CLOSE_WAIT)
        {
            ...
            switchboard6_close(sockets[fd].conn_id, our_mac, our_ip);
        }
    }
    else if (sockets[fd].kind == SOCK_TCP && !sockets[fd].listening && sockets[fd].conn_id != LOCKBOX_CAPACITY)
    {
