...
static packets6_state_t entries[LOCKBOX6_CAPACITY];
...
static int resend(packets6_state_t *e, uint32_t conn_id, const char *reason)
{
    uint32_t pass_id;
    ...
    {
        retransmit_total++;
        kprintf("[Scheduler6] slot %u: %s, resending seq %u\n", conn_id, reason, e->seq);
        return 1;
    }

    kprintf("[Scheduler6] slot %u: retransmit blocked by Bailiff, will retry\n", conn_id);
    return 0;
}

int scheduler6_track(uint32_t conn_id, uint32_t seq, const uint8_t *frame, uint16_t len)
{
    if (!valid_id(conn_id) || len == 0 || len > SCHED6_MAX_FRAME)
        return 0;

    packets6_state_t *e = &entries[conn_id];
    ...
    e->state = PACKET6_STATE_RETRY;
    e->retransmitted = 0;
    e->dup_ack_count = 0;

    if (e->rto == 0)
        e->rto = SCHED6_INITIAL_RTO;

    kprintf("[Scheduler6] slot %u: tracking seq %u, %u bytes, waiting for ack\n", conn_id, seq, len);
...
void scheduler6_ack(uint32_t conn_id, uint32_t acked_seq)
{
    ...
    if (e->state != PACKET6_STATE_RETRY)
        return;
    ...
    if (acked_seq >= e->seq + e->length)
    {
        if (!e->retransmitted)
        {
            ...
            if (e->rto < SCHED6_MIN_RTO)
                e->rto = SCHED6_MIN_RTO;
            if (e->rto > SCHED6_MAX_RTO)
                e->rto = SCHED6_MAX_RTO;

            kprintf("[Scheduler6] slot %u: rtt sample %u ticks, rto now %u\n", conn_id, sample, e->rto);
        }

        kprintf("[Scheduler6] slot %u: ack for seq %u arrived, clearing retransmit timer\n", conn_id, acked_seq);
        e->state = PACKET6_STATE_IDLE;
        e->retry_track = 0;
        e->dup_ack_count = 0;
    ...
    if (acked_seq == e->last_ack_seen)
    {
        ...
        kprintf("[Scheduler6] slot %u: duplicate ack for %u (%u in a row)\n", conn_id, acked_seq, e->dup_ack_count);

        if (e->dup_ack_count >= SCHED6_DUP_ACK_THRESHOLD)
        {
...
void scheduler6_tick(void)
{
    ...
    for (uint32_t i = 0; i < LOCKBOX6_CAPACITY; i++)
    {
        ...
        if (e->retry_track >= SCHED6_RETRY_LIMIT)
        {
            kprintf("[Scheduler6] slot %u: gave up after %u retries, seq %u never acked\n",
                    i, e->retry_track, e->seq);

            e->state = PACKET6_STATE_FAILURE;
            giveup_total++;
        ...
        e->retransmitted = 1;
        e->rto = (e->rto * 2 > SCHED6_MAX_RTO) ? SCHED6_MAX_RTO : e->rto * 2;
        e->retry_track++;
        e->start_timer = now;
    }
}
...
uint16_t scheduler6_bytes_in_flight(uint32_t conn_id)
{
    ...
    return (entries[conn_id].state == PACKET6_STATE_RETRY) ? entries[conn_id].length : 0;
}
