...
int exec_user(void *binary, uint32_t size)
{
    ...
    task->context_esp = build_initial_stack((uint8_t *)kstack,
                                            hdr->entry_point, 0x1B, 0x23,
                                            USER_STACK_TOP);
    ...
    task_log_event(current_task, EVT_EXEC, task->pid);
...
int sys_exec(const char *path)
{
    ...
    strncpy(task->name, path, TASK_NAME_LEN - 1);
